 ...
 * @namespace
 */
export const StdTransFunc = (() => ({
  /**
   * Transition to the next scene immediately.
   * @member {function}
   * @memberof StdTransFunc
   */
   ...
   * @memberof StdTransFunc
   */
  silhouetteIn: (duration, focusX, focusY, draw) => (prev, next, counter, painter) => {
   ...
   * @memberof StdTransFunc
   */
  silhouetteOut: (duration, focusX, focusY, draw) => (prev, next, counter, painter) => {
    const progress = counter / duration;
    const maxScale = Math.max(next.width, next.height);
    const scale = Math.pow(maxScale, 1 - 2 * progress);
    painter.clear();
